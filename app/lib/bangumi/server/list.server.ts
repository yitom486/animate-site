...
export async function fetchAnimeList(searchParams: URLSearchParams): Promise<AnimeListResult> {
  ...
  switch (query.view) {
    case "calendar": {
      const calendar = await fetchCalendarSchedule();
      return {
        items: [],
    ...
    case "tag":
    case "search": {
      const result = await fetchSearchList(query, offset);
      items = result.items;
      total = result.total;
